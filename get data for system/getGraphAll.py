...
def getGraph(DBname, ID, hop, field, atrrVals, fieldNum, other):
    ...
    if os.path.exists(filePath):
        ...
        if other:
            ...
            # if fieldNum and len(keywords_most_list) < fieldNum:
            #     not_nodes_all.append(other)
            # elif not fieldNum and len(keywords_most_list) < 10:
            # not_nodes_all.append(other)
            ...
            for i in not_edges_all:
                if '-1' == i['source'] or '-1' == i['target']:
                    not_nodes_all.append(other)
                    break
        else:
            not_nodes_all = sample_nodes_all + not_sample_nodes_all
            not_edges_all = sample_edges_all + not_sample_edges_all + other_edges_all
...
if __name__ == '__main__':
    ...
    param = {'dbname': 'co-authorship', 'field': 'num_citation', 'SCList':[(0, 70, 13), (70, 326, 13), (326, 653, 13), (653, 14830, 13)]}
    # param = {'dbname': 'citation', 'field': 'keywords', 'SCList': ['graph drawing', 'computer graphics', 'information visualization',  'network visualization', 'dynamic graph visualization']}

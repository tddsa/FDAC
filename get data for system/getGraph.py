...
def getGraph_NonNum_Without_Other(DBname, ID, hop, field, atrrVals, other):
    ...
    if os.path.exists(filePath):
        ...
        for j in sample_nodes:
            ...
            node = {'label':label, 'id':j['id'], 'name':j['name'], field:keyword_j}
            label += 1
            sample_nodes_all.append(node)
        ...
        for i in (sample_nodes_all+not_sample_nodes_all):
            ...
            for j in neigbors[1]:
                if not_node_map_attr[j] == attr:
                    ...
                    unsame += 1
            i[field] = {'attribute':attr, 'same':same, 'unsame':unsame}
...
def getGraph_Num_Without_Other(DBname, ID, hop, field, atrrVals, other):
    db = DBname.split('/')[1].split('.')[0]
    if not other:
        otherLabel = '0'
    else:
...
def getGraph_NonNum_With_Other(DBname, ID, hop, field, atrrVals, other):
    db = DBname.split('/')[1].split('.')[0]
    if not other:
        otherLabel = '0'
    else:
...
def getGraph_Num_With_Other(DBname, ID, hop, field, atrrVals, other):
    db = DBname.split('/')[1].split('.')[0]
    if not other:
        otherLabel = '0'
    else:
    ...
    if os.path.exists(filePath):
        ...
        # 保存结果
        json_graph = json.dumps(not_sample, indent=4, ensure_ascii=False) 
        with open(filePath, 'w', encoding='utf-8') as f:
            f.write(json_graph)
            f.close()
    return json_graph

def getGraph(DBname, ID, hop, field, atrrVals, fieldNum, other):
    if not fieldNum:    
        if not other:
            return getGraph_NonNum_Without_Other(DBname, ID, hop, field, atrrVals, other)
        else:
            return getGraph_NonNum_With_Other(DBname, ID, hop, field, atrrVals, other)
    else:
        if not other:
            return getGraph_Num_Without_Other(DBname, ID, hop, field, atrrVals, other)
        else:
            return getGraph_Num_With_Other(DBname, ID, hop, field, atrrVals, other)
            
    
if __name__ == '__main__':
    # authors keywords n_citation num_citation interests [(0, 70, 13), (70, 326, 13), (326, 653, 13)]
    param = {'dbname': 'co-authorship', 'field': 'num_citation', 'SCList':[(0, 70, 13), (70, 326, 13), (326, 653, 13)]}
    ...
    ID = '586652'
    hop = 1
    atrrVals = param['SCList']
    field = param["field"]
    # fieldNum = None
    fieldNum = 4
    # other = True
    other = False
    res = getGraph(db_name, ID, hop, field, atrrVals, fieldNum, other)
    # print(res)
    print('end')
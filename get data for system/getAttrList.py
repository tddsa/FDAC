...
def getDataset(dbname, ID, hop, field, fieldNum):
    
    return 1

def getAttrList(dbname, ID, hop, field, fieldNum=None):
    if fieldNum:
        return getDataset_numerical_Attributes(dbname, ID, hop, field, fieldNum)
    else:
        return getDataset_Non_numerical_Attributes(dbname, ID, hop, field)


if __name__ == "__main__":
    # authors keywords n_citation num_citation interests
    param = {'dbname': 'co-authorship', 'field': 'num_citation'}

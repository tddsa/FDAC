...
function center$1(x, y) {
  ...
  function force() {
    ...
    // console.log('community', 'center', x,y, 'now', sx/n, sy/n);
...
function link(links) {
  ...
  function initializeDistance() {
    ...
    for (var i = 0, n = links.length; i < n; ++i) {
      distances[i] = +distance(links[i], i, links) * 5;
//      distances[i] = +distance(links[i], i, links) ;
      sourceR = links[i].source.num;
      targetR = links[i].target.num;
      distances[i] *= Math.sqrt((sourceR + targetR) / total)
    }
...
function manyBody() {
  ...
  function initialize() {
    ...
    for (i = 0; i < n; ++i) {
      node = nodes[i];
      var strengthRate = Math.sqrt(node.num / allNum);
      // var strengthRate = node.num / allNum;
      strengths[node.index] = +strength(node, i, nodes) * strengthRate * 200;
      // strengths[node.index] = +strength(node, i, nodes) * 100;
